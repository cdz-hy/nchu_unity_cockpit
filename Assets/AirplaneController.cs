...
public class AirplaneController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 如果 x、y 或 z 方向变化过大（大于10单位），直接跳转到目标位置
        if (Mathf.Abs(targetPosition.x - transform.position.x) > 50f ||
            Mathf.Abs(targetPosition.z - transform.position.z) > 50f ||
            Mathf.Abs(targetPosition.y - transform.position.y) > 50f)
        {
