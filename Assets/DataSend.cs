...
public class DataSend : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float throttleLever1 = DataCenter.Instance.throttleLever1;
        float throttleLever2 = DataCenter.Instance.throttleLever2;
        ...
        XPlaneConnectNative.sendDREF(socket, controllerDref3, new float[] { rollControl }, 1);

        string throttleLever = "sim/cockpit2/engine/actuators/throttle_ratio_all";
        XPlaneConnectNative.sendDREF(socket, throttleLever, new float[] { (throttleLever1 + throttleLever2) / 2 }, 1);
    }
