...
public class RightSecondRocket : MonoBehaviour
{
    private int flaps = 0;
    void Start()
    {
    ...
    void Update()
    {
        ...
        if (flaps == 0)
        {
            transform.localRotation = Quaternion.Lerp(transform.localRotation, Quaternion.Euler(0.8f, 0, 0), Time.deltaTime);
        }
        if (flaps == 1)
        {
            transform.localRotation = Quaternion.Lerp(transform.localRotation, Quaternion.Euler(3.625f, 0, 0), Time.deltaTime);
        }
        if (flaps == 2)
        {
